"""
SimLingo-TCP Decoder:
  VLM encoder (SimLingo) → lang_embed → TCP-style GRU decoder → waypoints + route

定位：保留 TCP 的 autoregressive GRU 解码思想，加入 route head，对齐 SimLingo loss。
"""
...
class TCPDecoder(nn.Module):
    """TCP-style autoregressive decoder + route head，每步 GRU 注入 lang_embed + state_embed。"""

    def __init__(self, hidden_size=896, lang_dim=128, state_dim=128,
                 pred_len=10, num_route_points=20):
        super().__init__()
        self.pred_len = pred_len
        self.hidden_size = hidden_size

        # Join: mm_global + vis_pooled + lang_embed + state_embed
        ...
        # GRU: wp(2) + tp(2) + lang(lang_dim) + state(state_dim)
        gru_in = 2 + 2 + lang_dim + state_dim
        ...
        # Route head: j + att_vis + lang_embed + state_embed
        route_in = hidden_size * 2 + lang_dim + state_dim
        ...
        # Spatial attention: vis_spatial + lang_embed + state_embed
        att_in = hidden_size + lang_dim + state_dim
        self.att_fc = nn.Sequential(
            nn.Linear(att_in, 256), nn.ReLU(inplace=True),
            nn.Linear(256, 1),
        )

        # State encoder: speed(1) + tp(2) + command(6) = 9 → state_dim
        self.state_encoder = nn.Sequential(
            nn.Linear(9, state_dim), nn.ReLU(inplace=True),
            nn.Linear(state_dim, state_dim), nn.ReLU(inplace=True),
        )

    def forward(self, mm_global, vis_spatial, lang_embed, speed, tp, cmd):
        """
        mm_global:   [B, H]  VLM 多模态全局特征
        vis_spatial: [B, N, H]  ViT 空间特征
        lang_embed:  [B, D]  VLM 提取的语义向量
        speed:       [B, 1]
        tp:          [B, 2]  target_point
        cmd:         [B, 6]  one-hot command
        """
        B, N, H = vis_spatial.shape
        D = lang_embed.shape[1]
        device = vis_spatial.device

        # ---- State embedding (TCP 的 measurement branch) ----
        state = torch.cat([speed, tp, cmd], dim=1)  # [B, 9]
        state_embed = self.state_encoder(state)      # [B, state_dim]
        ...
        # ---- 语言+状态感知的空间注意力 ----
        lang_state = lang_embed.unsqueeze(1).expand(-1, N, -1)   # [B, N, D]
        state_exp = state_embed.unsqueeze(1).expand(-1, N, -1)   # [B, N, S]
        att_in = torch.cat([vis_spatial, lang_state, state_exp], dim=-1)
        att_w = self.att_fc(att_in).squeeze(-1).softmax(dim=1)    # [B, N]
        att_vis = (vis_spatial * att_w.unsqueeze(-1)).sum(dim=1)  # [B, H]

        # ---- Trajectory GRU (每步注入 lang + state) ----
        x = torch.zeros(B, 2, device=device, dtype=j.dtype)
        z = j
        pred_wp_list = []
        for _ in range(self.pred_len):
            gru_in = torch.cat([x, tp, lang_embed, state_embed], dim=1)
        ...
        # ---- Route head ----
        route_feat = torch.cat([j, att_vis, lang_embed, state_embed], dim=1)
...
class TCPLangModel(pl.LightningModule):
    def __init__(self, cfg_data_module, processor, cache_dir, **cfg):
        ...
        self.tcp_decoder = TCPDecoder(
            ...
            lang_dim=lang_dim, state_dim=state_dim,
    ...
    def forward_model(self, driving_input: DrivingInput):
        ...
        cmd = driving_input.prompt.language_string  # parse command from prompt text
        # One-hot encode command from text
        ...
        cmd_onehot = cmd_onehot.to(device)

        # ---- 7) TCP decoder ----
        pred_wp, pred_route = self.tcp_decoder(
            mm_global, vis_spatial, lang_embed, speed, tp, cmd_onehot)
        return pred_wp, pred_route

    # ---- Loss & steps (unchanged from SimLingo) ----
    ...
    def _visualise_step(self, batch, output, step):
        ...
        # 提取 lang_embed (last run stored in self._last_lang)
