"""
TCP v3 — ResNet-34 TCP + SimLingo language-state unified fusion（保留 command）。

核心：
  - fusion_mlp: lang_embed(128) + state_embed(128) → fused_embed(256)  只算一次
  - Join 使用原始 lang_embed + state_embed（不预融合）
  - GRU / Route 使用 fused_embed（规划阶段统一语义）
  - Attention 仅用 lang_embed（感知阶段语言指导"看哪里"）
"""
...
class TCPModelV3(BaseResNetTCP):
    """TCP + unified language-state fusion with command one-hot."""
