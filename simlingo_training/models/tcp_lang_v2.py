"""
TCP_lang v2 — 去掉 command one-hot，缩小 state_dim。

改动 vs v1：
  - 删除 command one-hot，state 输入从 9 维降到 3 维 (speed + tp)
  - state_dim: 128 → 32
  - lang_embed(128) + state_embed(32) 分别注入 join / attention / GRU / route head
"""
...
class TCPDecoderV2(nn.Module):
    """v2 decoder：去掉 command，state 只含 speed+tp，分散注入。"""

    def __init__(self, hidden_size=896, lang_dim=128, state_dim=32,
        ...
        # GRU: x(2) + tp(2) + lang_embed + state_embed
        ...
        # State encoder: speed(1) + tp(2) = 3 → state_dim (32)
    ...
    def forward(self, mm_global, vis_spatial, lang_embed, speed, tp):
        ...
        # ---- State embedding（仅 speed + tp，无 command）----
...
class TCPLangModelV2(pl.LightningModule):
    """TCP_lang v2：去 command，state 仅 speed+tp (32d)，语言承担导航意图。"""
    ...
    def _visualise_step(self, batch, output, step):
        ...
        save_dir = Path("visualise_tcp_lang_v2")
