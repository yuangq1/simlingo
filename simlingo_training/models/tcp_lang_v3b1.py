"""
TCP_lang v3-b1 — LLM-output-only planning decoder（无结构化 state 输入）。

改动 vs v3-a：
  - 删除 state_encoder、fusion_mlp、raw speed/tp 输入
  - lang_proj 直接输出 planning_embed 256（替代 v3-a 的 lang_embed 128）
  - decoder 仅接收 planning_embed，GRU 不再拼接 raw tp
  - 验证：VLM hidden states 是否已充分编码所有规划所需信息
"""
...
class TCPDecoderV3B1(nn.Module):
    """v3-b1 decoder：只使用 VLM planning_embed，无结构化 speed/tp 输入。"""
    ...
    def __init__(self, hidden_size=896, planning_dim=256,
        ...
        # GRU: x(2) + planning_embed(planning_dim) — 不拼接 raw tp
        gru_in = 2 + planning_dim
    ...
    def forward(self, mm_global, vis_spatial, planning_embed):
        """
        ...
        # ---- Trajectory GRU: 每步注入 planning_embed（无 raw tp）----
        x = torch.zeros(B, 2, device=device, dtype=j.dtype)
        z = j
        pred_wp_list = []
        for _ in range(self.pred_len):
            gru_in = torch.cat([x, planning_embed], dim=1)
...
class TCPLangModelV3B1(pl.LightningModule):
    """TCP_lang v3-b1：LLM-output-only，验证 VLM hidden states 是否足够。"""
    ...
    def __init__(self, cfg_data_module, processor, cache_dir, **cfg):
        ...
        self.tcp_decoder = TCPDecoderV3B1(
    ...
    def forward_model(self, driving_input: DrivingInput):
        ...
        self._last_lang = planning_embed.detach()
        del last_hidden, llm_out, prompt_embeds
        vis_spatial = img_feat_2d[:sel.sum()].reshape(B, -1, img_feat.shape[-1])  # [B, N_i, 896]

        # ---- 6) TCP decoder（无 speed/tp）----
        pred_wp, pred_route = self.tcp_decoder(mm_global, vis_spatial, planning_embed)
        return pred_wp, pred_route
    ...
    def _visualise_step(self, batch, output, step):
        ...
        save_dir = Path("visualise_tcp_lang_v3b1")
