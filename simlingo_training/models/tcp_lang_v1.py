"""
TCP_lang v1 — VLM 语言增强 TCP（保留 command one-hot）。

改动 vs 原始 TCP：
  - 新增 VLM/LLM encoder → mm_global → lang_proj → lang_embed 128
  - 保留 command one-hot (9维 input → state_embed 128)
  - lang_embed + state_embed 分散注入 join / attention / GRU / route head
"""
...
class TCPDecoderV1(nn.Module):
    """v1 decoder：mm_global + vis_pooled + lang_embed + state_embed 全部参与。"""
    ...
    def __init__(self, hidden_size=896, lang_dim=128, state_dim=128,
        ...
        # GRU: x(2) + tp(2) + lang_embed(lang_dim) + state_embed(state_dim)
    ...
    def forward(self, mm_global, vis_spatial, lang_embed, speed, tp, cmd_onehot):
        ...
        # ---- 语言+状态感知的空间注意力 ----
        lang_exp = lang_embed.unsqueeze(1).expand(-1, N, -1)    # [B, N, D]
        state_exp = state_embed.unsqueeze(1).expand(-1, N, -1)  # [B, N, S]
        att_in = torch.cat([vis_spatial, lang_exp, state_exp], dim=-1)
...
class TCPLangModelV1(pl.LightningModule):
    """TCP_lang v1：VLM 语言注入 + command one-hot 导航先验。"""
    ...
    def forward_model(self, driving_input: DrivingInput):
        ...
        cmd = driving_input.prompt.language_string
    ...
    def _visualise_step(self, batch, output, step):
        ...
        save_dir = Path("visualise_tcp_lang_v1")
