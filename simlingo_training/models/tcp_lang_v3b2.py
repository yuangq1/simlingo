"""
TCP_lang v3-b2 — Planning embed + raw target_point in GRU only（当前版本）。

改动 vs v3-b1：
  - 保留 planning_embed 256 作为统一规划表示
  - 在 GRU 中恢复 raw target_point：gru_in = x + tp + planning_embed
  - tp 只进入 GRU，不进入 join / attention / route head
  - 验证：v3-b1 变差是否是 GRU 缺少 raw tp 导致的
"""
...
class TCPDecoderV3B2(nn.Module):
    """v3-b2 decoder：planning_embed 驱动，GRU 中保留 raw target_point。"""

    def __init__(self, hidden_size=896, planning_dim=256,
        ...
        # Join: mm_global + vis_pooled + planning_embed
        join_in = hidden_size * 2 + planning_dim
        self.join = nn.Sequential(
            nn.Linear(join_in, hidden_size), nn.ReLU(inplace=True),
            nn.Linear(hidden_size, hidden_size), nn.ReLU(inplace=True),
        )

        # GRU: current waypoint x(2) + raw target_point tp(2) + planning_embed(256)
        gru_in = 2 + 2 + planning_dim
        ...
        # Route head: j + att_vis + planning_embed（不使用 raw tp）
        route_in = hidden_size * 2 + planning_dim
        ...
        # Spatial attention: vis_spatial + planning_embed
        att_in = hidden_size + planning_dim
        self.att_fc = nn.Sequential(
            nn.Linear(att_in, 256), nn.ReLU(inplace=True),
            nn.Linear(256, 1),
        )

    def forward(self, mm_global, vis_spatial, planning_embed, tp):
        """
        mm_global:      [B, H]      VLM 多模态全局特征
        vis_spatial:    [B, N, H]   ViT 空间特征
        planning_embed: [B, 256]    VLM 输出的统一规划表示
        tp:             [B, 2]      raw target_point，只用于 GRU 解码
        """
        ...
        # ---- Join: 多模态融合 ----
        j = self.join(torch.cat([mm_global, vis_pooled, planning_embed], dim=1))

        # ---- planning-aware spatial attention ----
        plan_exp = planning_embed.unsqueeze(1).expand(-1, N, -1)  # [B, N, 256]
        att_in = torch.cat([vis_spatial, plan_exp], dim=-1)
        att_w = self.att_fc(att_in).squeeze(-1).softmax(dim=1)    # [B, N]
        att_vis = (vis_spatial * att_w.unsqueeze(-1)).sum(dim=1)  # [B, H]

        # ---- Trajectory GRU: 每步注入 planning_embed + raw target_point ----
        x = torch.zeros(B, 2, device=device, dtype=j.dtype)
        z = j
        pred_wp_list = []
        for _ in range(self.pred_len):
            gru_in = torch.cat([x, tp, planning_embed], dim=1)
        ...
        # ---- Route head: 不直接使用 raw tp ----
        route_feat = torch.cat([j, att_vis, planning_embed], dim=1)
...
class TCPLangModelV3B2(pl.LightningModule):
    """TCP_lang v3-b2：planning_embed 驱动，GRU 保留 raw tp（当前默认版本）。"""
    ...
    def __init__(self, cfg_data_module, processor, cache_dir, **cfg):
        ...
        # Config
        planning_dim = getattr(self, 'planning_dim', 256)
        pred_len = getattr(self, 'pred_len', 11)
        route_pts = cfg_data_module.get('num_route_points', 20)

        self.tcp_decoder = TCPDecoderV3B2(
            hidden_size=self.language_model.hidden_size,
            planning_dim=planning_dim,
            pred_len=pred_len - 1,
            num_route_points=route_pts)

        # lang_proj: mm_global 896 → 512 → planning_dim 256
        self.lang_proj = nn.Sequential(
            nn.Linear(self.language_model.hidden_size, 512),
            nn.ReLU(inplace=True),
            nn.Linear(512, planning_dim))
    ...
    def forward(self, example, return_language=None, prompt_ids=None):
        """兼容 SimLingo 的 forward 接口（供 VisualiseCallback 调用）"""
        try:
    ...
    def forward_model(self, driving_input: DrivingInput):
        ...
        last_hidden = llm_out.hidden_states[-2]  # [B, L, H] 倒数第二层

        # ---- 5) Extract features ----
        mask = prompt_valid.to(last_hidden.dtype).unsqueeze(-1)
        mm_global = (last_hidden * mask).sum(dim=1) / mask.sum(dim=1).clamp(min=1.0)  # [B, 896]
        planning_embed = self.lang_proj(mm_global)  # [B, 256]
        self._last_lang = planning_embed.detach()   # 供可视化
        del last_hidden, llm_out, prompt_embeds      # 释放显存
        vis_spatial = img_feat_2d[:sel.sum()].reshape(B, -1, img_feat.shape[-1])  # [B, N_i, 896]

        # ---- 6) TCP decoder ----
        tp = driving_input.target_point.view(B, -1)[:, :2]  # [B, 2]，仅用于 GRU
        pred_wp, pred_route = self.tcp_decoder(
            mm_global, vis_spatial, planning_embed, tp)
        return pred_wp, pred_route
    ...
    def _visualise_step(self, batch, output, step):
        """每1000步：保存 embedding + waypoints 对比图。"""
        import matplotlib
        matplotlib.use('Agg')
        ...
        save_dir = Path("visualise_tcp_lang")
        ...
        if le is not None:
            ...
            ax.set_title(f'planning_embed [{len(le)} dims] (step {step})\n'
            ...
            ax.set_title('planning_embed (not captured)')
